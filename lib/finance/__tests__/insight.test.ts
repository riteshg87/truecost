...
describe("recommend", () => {
  ...
  it("flags the case the product exists for: lowest rate is not cheapest", () => {
    const r = rec([
      offer({ id: "a", lender: "Cheap rate", ratePct: 8.4, processingFee: { mode: "flat", value: 150000 } }),
      offer({ id: "b", lender: "Cheap fee", ratePct: 8.5, processingFee: { mode: "flat", value: 25000 } }),
    ]);

    expect(r.best.offer.lender).toBe("Cheap fee");
    expect(r.headlineMisleads).toBe(true);
    expect(r.driver).toBe("charges");
    // Their better rate hands a little back, so the rate term goes negative.
    expect(r.rateEffect).toBeLessThan(0);
    expect(r.rateEffect + r.chargesEffect).toBeCloseTo(r.aprGap, 6);
  });

  it("does not cry wolf when the lowest rate genuinely wins", () => {
    const r = rec([
      offer({ id: "a", lender: "Low", ratePct: 8.3, processingFee: { mode: "flat", value: 20000 } }),
      offer({ id: "b", lender: "High", ratePct: 9.6, processingFee: { mode: "flat", value: 20000 } }),
    ]);
  ...
  it("splits the gap into rate and charges, and the two account for it", () => {
    const r = rec([
      offer({ id: "a", lender: "A", ratePct: 8.4, processingFee: { mode: "flat", value: 20000 } }),
      offer({ id: "b", lender: "B", ratePct: 8.62, processingFee: { mode: "flat", value: 110000 } }),
    ]);
  ...
  it("the fee waiver it quotes actually draws the runner-up level", () => {
    const offers = [
      offer({ id: "a", lender: "A", ratePct: 8.5, processingFee: { mode: "flat", value: 20000 } }),
      offer({ id: "b", lender: "B", ratePct: 8.45, processingFee: { mode: "flat", value: 140000 } }),
    ];
    const r = rec(offers);
  ...
  it("reports no waiver when the rate alone already loses", () => {
    const r = rec([
      offer({ id: "a", lender: "A", ratePct: 8.5, processingFee: { mode: "flat", value: 0 } }),
      offer({ id: "b", lender: "B", ratePct: 11.5, processingFee: { mode: "flat", value: 0 } }),
    ]);
  ...
  it("the rate cut it quotes actually draws the runner-up level", () => {
    const r = rec([
      offer({ id: "a", lender: "A", ratePct: 8.5, processingFee: { mode: "flat", value: 20000 } }),
      offer({ id: "b", lender: "B", ratePct: 9.7, processingFee: { mode: "flat", value: 20000 } }),
    ]);
  ...
  it("calls a dead heat too close and offers no levers", () => {
    const r = rec([
      offer({ id: "a", lender: "A", ratePct: 8.5 }),
      offer({ id: "b", lender: "B", ratePct: 8.5 }),
    ]);
  ...
  it("holds up across differing amounts, where charge load matters not rupees", () => {
    const r = rec([
      offer({ id: "a", lender: "Small", amount: 2000000, ratePct: 8.5, processingFee: { mode: "flat", value: 20000 } }),
      offer({ id: "b", lender: "Large", amount: 8000000, ratePct: 8.5, processingFee: { mode: "flat", value: 80000 } }),
    ]);
  ...
  it("treats a flat quote on its reducing equivalent when judging the headline", () => {
    const r = rec([
      offer({ id: "a", lender: "Reducing", ratePct: 8.5 }),
      offer({ id: "b", lender: "Flat", ratePct: 6, rateType: "flat" }),
    ]);

    // 6% flat over twenty years is ~9.26% reducing, so the lower-looking number
    // must not be credited as the lowest rate.
